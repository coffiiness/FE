...
<template>
  <section class="chat-panel h-[420px] max-h-[420px] flex flex-col rounded-xl px-5 py-5">
    <div class="flex items-start justify-between gap-4 mb-4">
      <div>
        <h3 class="text-lg font-display font-black text-slate-950">익명 라운지</h3>
        <p class="mt-1 text-xs text-slate-500">같은 워크스페이스 사람들과 가볍게 이야기할 수 있어요.</p>
      </div>
      <div class="flex items-center gap-2 shrink-0">
        <span class="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-black tracking-[0.04em] text-slate-500">
          최근 {{ messageCountLabel }}
        </span>
        <button
          v-if="props.showCloseButton"
          type="button"
          class="w-8 h-8 inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-slate-700 transition"
          @click="emit('close')"
        >
    ...
    <div
      ref="bodyRef"
      class="chat-body min-h-0 flex-1 overflow-y-auto custom-scrollbar px-3 py-3 space-y-2"
    >
      ...
      >
        <article
          ...
          :class="message.mine ? 'chat-bubble-mine text-white' : 'chat-bubble-other text-slate-700'"
        >
          <div class="flex items-center gap-2 mb-1">
            <span
              class="text-[11px] font-bold"
              :class="message.mine ? 'text-teal-50/90' : 'text-teal-800'"
            >
              {{ message.alias }}
            </span>
            <span
    ...
    <div class="mt-4">
      <div class="flex items-end gap-3">
        <textarea
          ...
          rows="2"
          maxlength="500"
          class="flex-1 resize-none rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-teal-400 focus:ring-4 focus:ring-teal-100"
          placeholder="익명으로 메시지를 남겨보세요."
          @keydown.enter.exact.prevent="sendMessage"
        />
        <button
          type="button"
          class="shrink-0 rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-black text-white transition hover:bg-slate-900 disabled:cursor-not-allowed disabled:bg-slate-300"
          :disabled="!canSend"
          @click="sendMessage"
        >
          {{ sending ? '전송 중' : '보내기' }}
