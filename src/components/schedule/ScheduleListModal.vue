...
const eventItems = computed(() => (Array.isArray(props.events) ? props.events : []))
...
const getEventStyle = (type) => {
  switch (type) {
    case 'INTERVIEW':
      return { bg: 'bg-indigo-50', border: 'border-indigo-100', text: 'text-indigo-700', badge: 'bg-white border-indigo-200 text-indigo-700' }
    case 'MEETING':
      return { bg: 'bg-amber-50', border: 'border-amber-100', text: 'text-amber-700', badge: 'bg-white border-amber-200 text-amber-700' }
    case 'BUSINESS':
      return { bg: 'bg-emerald-50', border: 'border-emerald-100', text: 'text-emerald-700', badge: 'bg-white border-emerald-200 text-emerald-700' }
    case 'VACATION':
      return { bg: 'bg-rose-50', border: 'border-rose-100', text: 'text-rose-700', badge: 'bg-white border-rose-200 text-rose-700' }
    case 'OTHERS':
      return { bg: 'bg-slate-50', border: 'border-slate-100', text: 'text-slate-700', badge: 'bg-white border-slate-200 text-slate-700' }
    default:
      return { bg: 'bg-brand-50', border: 'border-brand-100', text: 'text-brand-700', badge: 'bg-white border-brand-200 text-brand-700' }
  }
}

const toAlphaColor = (hexColor, alpha) => {
  const normalized = String(hexColor || '').trim()
  if (!/^#([0-9a-fA-F]{6})$/.test(normalized)) return ''
  return `${normalized}${alpha}`
}

const getEventAccentStyle = (evt) => {
  const color = typeof evt?.color === 'string' ? evt.color.trim() : ''
  if (!color) return null

  return {
    container: {
      backgroundColor: toAlphaColor(color, '10'),
      borderColor: toAlphaColor(color, '40')
    },
    badge: {
      borderColor: toAlphaColor(color, '55'),
      color
    },
    type: {
      color,
      backgroundColor: toAlphaColor(color, '12'),
      borderColor: toAlphaColor(color, '18')
    }
  }
}
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-[400px] max-w-[90%] flex flex-col max-h-[600px] animate-fade-in-up border border-slate-200 overflow-hidden">
          <div class="bg-slate-50 p-5 border-b border-slate-100 flex justify-between items-center flex-none">
            <div>
              ...
              <p class="text-xs text-slate-500 font-medium mt-1">총 {{ eventItems.length }}건의 일정</p>
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar">
            <div v-if="eventItems.length === 0" class="flex flex-col items-center justify-center h-40 text-slate-400 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
              <p class="font-medium text-sm">등록된 일정이 없습니다.</p>
            </div>

            <div
              v-for="evt in eventItems"
              :key="evt.id"
              ...
              :style="getEventAccentStyle(evt)?.container || undefined"
              @click="$emit('edit', evt)"
            >
              <div
                class="flex-none flex flex-col items-center justify-center w-24 py-3 rounded-lg border shadow-sm"
                :class="getEventStyle(evt.type).badge"
                :style="getEventAccentStyle(evt)?.badge || undefined"
              >
                <span class="text-xs font-black tracking-tight text-center leading-relaxed whitespace-pre-line break-words">
              ...
              <div class="flex-1 min-w-0 py-1">
                ...
                <div class="flex items-center mt-2 gap-2 flex-wrap">
                  <span
                    class="text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider bg-white/60 border border-black/5 shrink-0"
                    :class="getEventStyle(evt.type).text"
                    :style="getEventAccentStyle(evt)?.type || undefined"
                  >
                    {{ typeLabel(evt.type) }}
                  </span>
