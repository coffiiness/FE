...
<template>
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <div v-else-if="!template" class="text-center py-12">
      ...
      <button @click="goBack" class="text-brand-600 hover:text-brand-700 font-medium">목록으로 돌아가기</button>
    </div>
    ...
    <template v-else>
      <div class="flex items-center justify-between">
        <div>
          <div class="flex items-center gap-3 mb-2">
            <button @click="goBack" class="p-1 text-gray-400 hover:text-gray-600 transition-colors">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <h1 class="text-2xl font-bold text-gray-900">{{ template.title || template.name }}</h1>
            <span
              :class="[
                isTemplateInUse(template.status)
          ...
          <p class="text-gray-500 text-sm ml-8">생성일 {{ template.createdAt || '-' }} · 수정일 {{ template.updatedAt || '-' }}</p>
        </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div v-else class="text-center py-8 text-gray-500">추가한 질문이 없습니다.</div>
      </div>
