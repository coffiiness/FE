...
const fetchRecruitments = async (search) => {
  ...
  try {
    ...
    error.value = '채용 공고를 불러오는 데 실패했습니다.'
    console.error(err)
  } finally {
    loading.value = false
  }
...
onMounted(async () => {
  try {
    ...
    companyName.value = matched?.companyName || companySlug.value
...
watch(searchQuery, (value) => {
  ...
  debounceTimer = setTimeout(() => fetchRecruitments(value.trim()), 300)
})
    ...
    <header class="border-b border-gray-200">
      <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 bg-brand-600 rounded flex items-center justify-center text-white font-bold">{{ company.logo }}</div>
      ...
      <div class="flex gap-8">
        <aside class="w-56 flex-shrink-0">
          <div class="sticky top-8">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <label v-for="type in careerTypes" :key="type.value" class="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
        ...
        <main class="flex-1">
          <div class="flex items-center justify-between mb-6">
            <p class="text-gray-600">총 <span class="font-semibold text-gray-900">{{ filteredJobs.length }}</span>개의 채용공고가 있습니다.</p>
            <div class="relative">
              <input
                v-model="searchQuery"
                type="text"
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div class="flex flex-wrap gap-2 mb-3">
                    <span class="px-2 py-1 bg-brand-50 text-brand-700 text-xs font-bold rounded">{{ getCareerText(job) }}</span>
                    <span v-if="job.targetCount" class="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded">{{ job.targetCount }}명 모집</span>
                  </div>
                </div>
                <div class="text-right ml-4 flex-shrink-0">
                  <p class="text-sm font-semibold" :class="getDeadlineText(job.endDate).includes('임박') ? 'text-rose-500' : 'text-gray-600'">{{ getDeadlineText(job.endDate) }}</p>
                  <p class="text-xs text-gray-400 mt-1">{{ formatDate(job.startDate) }} ~ {{ formatDate(job.endDate) }}</p>
                </div>
              </div>
            </div>

            <div v-if="filteredJobs.length === 0" class="text-center py-12 text-gray-500">채용 공고가 없습니다.</div>
          </div>
        </main>
      </div>
    </div>
